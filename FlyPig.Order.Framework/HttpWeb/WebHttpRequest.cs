    ...
    public class WebHttpRequest
    {
        ...
        public static string Send(string verb, string url, string postData)
        {
            ...
            catch (Exception ex)
            {
                ...
                //    HotelApi.Log.TFileLogManage.GetLog("request_error").Error(ex.Message.ToString());
            }
            return null;
        }


        public static string GetOrder(string url)
        {
            return SendOrder("GET", url, "");
        }

        public static string PostOrder(string url, string postData)
        {
            return SendOrder("POST", url, postData);
        }

        public static string SendOrder(string verb, string url, string postData)
        {
            System.Net.ServicePointManager.ServerCertificateValidationCallback =
                ((sender, certificate, chain, sslPolicyErrors) => true);
            HttpWebRequest req = (HttpWebRequest)WebRequest.Create(url);
